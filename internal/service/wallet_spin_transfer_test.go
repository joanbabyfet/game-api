...
func TestValidateSpinReplay(t *testing.T) {
	...
	order := &model.GameOrder{
		RequestID: "request-1", UID: 100, AgentID: 1, GameID: 7,
		SpinType: model.SpinTypeNormal, BetAmount: 1000,
	}

	if err := validateSpinReplay(order, req, claims, game, model.SpinTypeNormal); err != nil {
...
func TestValidateSpinReplayRejectsBetConflict(t *testing.T) {
	...
	err := validateSpinReplay(order, req, claims, game, model.SpinTypeNormal)
...
func TestValidateFreeSpinReplay(t *testing.T) {
	...
	if err := validateSpinReplay(order, req, claims, game, model.SpinTypeFreeSpin); err != nil {
		t.Fatalf("validateSpinReplay() error = %v", err)
	}
}
